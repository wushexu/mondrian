...
public class Thin {
  ...
  private static List<ThinCalculatedMember> convertCM(List<CalculatedMember> qcms) {
	...
	private static ThinAxis convertAxis(QueryAxis queryAxis, ThinQuery tq) {
		ThinQueryModel.AxisLocation loc = getLocation(queryAxis);
	...
	private static List<ThinFilter> convertFilters(List<IFilterFunction> filters) {
		...
		for (IFilterFunction f : filters) {
			if (f instanceof NameFilter) {
				...
				ThinFilter.FilterOperator type = ThinFilter.FilterOperator.LIKE;
				if(nf.getOp() != null && nf.getOp().equals("NOTEQUAL")){
					type = ThinFilter.FilterOperator.NOTEQUAL;
				}
				ThinFilter tf = new ThinFilter(ThinFilter.FilterFlavour.Name, type, ThinFilter.FilterFunction.Filter, expressions);
				tfs.add(tf);
			}
			if (f instanceof NameLikeFilter) {
				...
				ThinFilter tf = new ThinFilter(ThinFilter.FilterFlavour.NameLike, type, ThinFilter.FilterFunction.Filter, expressions);
				tfs.add(tf);
			}
			if (f instanceof GenericFilter) {
				...
				ThinFilter tf = new ThinFilter(ThinFilter.FilterFlavour.Generic, null, ThinFilter.FilterFunction.Filter, expressions);
				tfs.add(tf);
			}
			if (f instanceof NFilter) {
				...
				ThinFilter.FilterFunction type = ThinFilter.FilterFunction.valueOf(nf.getFunctionType().toString());
				ThinFilter tf = new ThinFilter(ThinFilter.FilterFlavour.N, null, type, expressions);
				tfs.add(tf);
			}
		}
		return tfs;
